...
export class SharedDoc {
  ...
  deleteComponent(name) {
    ...
    if (componentIndex === -1) throw new Error(`Could not find index of component to delete, ${name}`);

    // This should all work because we enforce having blank text before and after all components
    const textBlockBefore = /** @type {import('shared').BlockMap} */ (this.yarrayStore.y.get(componentIndex - 1));
    const textBlockAfter = /** @type {import('shared').BlockMap} */ (this.yarrayStore.y.get(componentIndex + 1));
